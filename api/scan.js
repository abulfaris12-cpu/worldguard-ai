export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({ ok: true, service: 'WorldGuard authorized scanner', ready: true, mode: 'low-impact-http' });
  }
  ...
  let url;
  try { url = new URL(target.trim()); } catch { return res.status(400).json({ error: 'Invalid target URL.' }); }
  if (!['https:', 'http:'].includes(url.protocol)) return res.status(400).json({ error: 'Only HTTP(S) targets are supported.' });

  // The deployed WorldGuard app is safe to assess as its own target.
  // Other authorized hosts must be explicitly configured by the deployment owner.
  const builtInHosts = ['worldguard-ai.vercel.app'];
  const configuredHosts = (process.env.ALLOWED_TARGET_HOSTS || '').split(',').map(x => x.trim().toLowerCase()).filter(Boolean);
  const allowed = [...new Set([...builtInHosts, ...configuredHosts])];
  const hostname = url.hostname.toLowerCase();
  if (!allowed.some(host => hostname === host || hostname.endsWith('.' + host))) {
    return res.status(403).json({ error: 'Target host is not allowlisted. Add your authorized staging host to ALLOWED_TARGET_HOSTS in Vercel.' });
  }
  ...
  async function request(path, method = 'GET') {
    ...
    try {
      const r = await fetch(u, { method, redirect: 'manual', signal: controller.signal, headers: { 'User-Agent': 'WorldGuard-AI-Authorized-Assessment/1.2', Accept: 'text/html,application/json,text/plain,*/*' } });
  ...
  try {
    ...
    const securityHeaders = [
      ['content-security-policy', 'Missing Content-Security-Policy', 'MEDIUM', 'Configure a Content-Security-Policy appropriate for the application.'],
      ...
      ['permissions-policy', 'Missing Permissions-Policy', 'LOW', 'Restrict browser capabilities the application does not need.']
    ];
    ...
    if (setCookie) {
      const problems = [];
      if (url.protocol === 'https:' && !/;\s*secure\b/i.test(setCookie)) problems.push('Secure');
      if (!/;\s*httponly\b/i.test(setCookie)) problems.push('HttpOnly');
      if (!/;\s*samesite\s*=/i.test(setCookie)) problems.push('SameSite');
      if (problems.length) findings.push({ id: 'cookie-flags', title: 'Session Cookie Security Flags', severity: 'MEDIUM', cvss: 5.4, evidence: `Set-Cookie was observed without: ${problems.join(', ')}.`, remediation: 'Apply Secure, HttpOnly and an appropriate SameSite policy to session cookies.' });
    }
    ...
    if (url.protocol === 'https:') {
      ...
      findings.push({ id: 'plaintext-http', title: 'Target Uses Plain HTTP', severity: 'MEDIUM', cvss: 5.9, evidence: 'The assessment target uses HTTP rather than HTTPS.', remediation: 'Use HTTPS for authenticated or sensitive application traffic.' });
    }

    // Passive same-origin discovery only; no discovered endpoint is attacked.
    if (root.text) {
      const patterns = [/(?:href|src|action)\s*=\s*["']([^"'#]+)["']/gi, /fetch\s*\(\s*["']([^"']+)["']/gi];
      for (const re of patterns) {
        let m;
        while ((m = re.exec(root.text)) !== null && endpoints.size < 25) {
          try { const u = new URL(m[1], url.origin); if (u.origin === url.origin && u.pathname.startsWith('/')) endpoints.add(u.pathname); } catch {}
        }
      }
    }
    ...
    for (const ep of exposedSensitive) findings.push({ id: `sensitive-${ep.path}`, title: 'Potentially Sensitive Endpoint Exposed', severity: 'MEDIUM', cvss: 5.3, evidence: `A same-origin path matching a sensitive naming pattern responded with HTTP ${ep.status}: ${ep.path}. This is a passive observation, not exploitation.`, remediation: 'Review whether the endpoint is intentionally public and enforce server-side authentication and authorization where required.' });

    const score = Math.max(0, Math.round(100 - findings.reduce((n, f) => n + (f.severity === 'HIGH' ? 20 : f.severity === 'MEDIUM' ? 12 : 5), 0)));
    return res.status(200).json({ ok: true, target: url.origin, status: root.r.status, durationMs: Date.now() - started, score, findings, endpoints: endpointResults, checked, methodology: 'Authorized low-impact passive discovery and GET response analysis. No credential attacks, destructive requests, fuzzing, or access-control bypass attempts.' });
  } catch (e) {
